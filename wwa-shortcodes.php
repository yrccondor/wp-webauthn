...
function wwa_list_shortcode($vals){
    ...
    $thead = '<div class="wwa-table-container"><table class="wwa-list-table"><thead><tr><th>'.__('Identifier', 'wwa').'</th><th>'.__('Type', 'wwa').'</th><th>'._x('Registered', 'time', 'wwa').'</th><th>'.__('Usernameless', 'wwa').'</th><th>'.__('Action', 'wwa').'</th></tr></thead><tbody class="wwa-authenticator-list">';
    $tfoot = '</tbody><tfoot><tr><th>'.__('Identifier', 'wwa').'</th><th>'.__('Type', 'wwa').'</th><th>'._x('Registered', 'time', 'wwa').'</th><th>'.__('Usernameless', 'wwa').'</th><th>'.__('Action', 'wwa').'</th></tr></tfoot></table></div>';

    $tip = '<p class="wwa-authenticator-list-usernameless-tip"></p>';
    $usernameless = wwa_get_option("usernameless_login");
    if($usernameless !== "true"){
        $tip = '<p class="wwa-authenticator-list-usernameless-tip">'.__('The site administrator has disabled usernameless login feature.','wwa').'</p>';
    }
    ...
    if(!current_user_can("read")){
        if($display === "true"){
            ...
            return $thead.'<tr><td colspan="6">'.__('You haven\'t logged in yet', 'wwa').'</td></tr>'.$tfoot;
        }else{
            return '';
        }
    }
    ...
    if(!isset(wwa_get_option("user_id")[$user_info->user_login])){
        ...
        return $thead.'<tr><td colspan="6">'.__('No registered authenticators', 'wwa').'</td></tr>'.$tfoot;
    }else{
    ...
    foreach($authenticator_list as $item){
        $html_list .= '<tr><td>'.$item["name"].'</td><td>'.($item["type"] === "none" ? __('Rename', 'wwa') : ($item["type"] === "none" ? __('Any authenticator', 'wwa') : ($item["type"] === "platform" ? __('Platform authenticator', 'wwa') : __('Roaming authenticator', 'wwa')))).'</td><td>'.$item["added"].'</td><td>'.($item["usernameless"] ? __('Ready','wwa').(($usernameless !== "true") ? __(' (Unavailable)','wwa') : "") : __('No','wwa')).'</td><td class="wwa-key-'.$item["key"].'"><a href="javascript:renameAuthenticator(\''.$item["key"].'\', \''.$item["name"].'\')">'.__('Rename', 'wwa').'</a> | <a href="javascript:removeAuthenticator(\''.$item["key"].'\', \''.$item["name"].'\')">'.__('Remove', 'wwa').'</a></td></tr>';
    }
    return $thead.$html_list.$tfoot.$tip;
}
